...
class FestivalShowTimeChart:
    def __init__(self, show_data):
        ...
        self.fig, ax = plt.subplots(figsize=(8, 0.5 * (len(grouped_shows) + 1)))
        ...
        for i, (_, group) in enumerate(grouped_shows):
            for _, row in group.iterrows():
                ...
                ax.barh(i, end - start, left=start, height=0.4, align='center', color='blue', alpha=0.8)
                ax.text(midpoint, i, row['Show'], ha='center', va='center', color='white')

        # Configure the chart layout
        ax.set_xlim(datetime.combine(dummy_date, datetime.strptime('09:00 PM', '%I:%M %p').time()),
                    datetime.combine(dummy_date + timedelta(days=1), datetime.strptime('06:00 AM', '%I:%M %p').time()))
        ...
        ax2.tick_params(left=False, labelleft=False, right=False, labelright=True)

        # Add time intervals at the bottom
        start_time = datetime.strptime('09:00 PM', '%I:%M %p').time()
        end_time = datetime.strptime('06:00 AM', '%I:%M %p').time()
